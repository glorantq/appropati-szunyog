...
def main():
    ...
    trainB = Rectangle(Point(kmToPx(trainsDistance) + 50, window.getHeight() / 2 - 25),
                       Point(kmToPx(trainsDistance) + 100, window.getHeight() / 2 + 25))
    trainB.setFill("blue")
    trainB.draw(window)
    ...
    debugLabel = Text(Point(window.getWidth() / 2, 40),
                      "A: {} km/h\nB: {} km/h\nFly: {} km/h\nWind: {} km/h".format(trainASpeed, trainBSpeed, flySpeed,
                                                                                   windSpeed))
    ...
    resultText = Text(Point(window.getWidth() / 2, window.getHeight() - 10),
                      "The fly travelled {} km!".format(round(distanceTravelled)))
